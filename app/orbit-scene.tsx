      ...
      <section className="workspace">
        {leftOpen && <aside className="left-rail">
          {mode === 'screening' ? <>
            ...
            <button className={`fleet-action ${fleetActive ? 'active' : ''}`} onClick={activateFleet}><span className="fleet-icon"><Satellite size={18} /></span><span><strong>Earth Observation Mission Fleet</strong><small>{fleetActive ? 'Active · debris encounter selected' : 'Activate nine verified missions'}</small></span>{fleetActive ? <Check size={16} /> : <Plus size={16} />}</button>
