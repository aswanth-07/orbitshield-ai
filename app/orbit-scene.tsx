...
function CollisionCandidateList({
  ...
  if (!risks.length) {
    return <div className="risk-empty"><ShieldCheck size={20} /><strong>No fleet-screening match</strong><p>This object has no event in the current Earth-observation SOCRATES view. That is not a guarantee of zero collision risk.</p></div>;
  }
      ...
      <section className="workspace">
        {leftOpen && <aside className="left-rail">
          {mode === 'screening' ? <>
            ...
            <button className={`fleet-action ${fleetActive ? 'active' : ''}`} onClick={activateFleet}><span className="fleet-icon"><Satellite size={18} /></span><span><strong>Earth Observation Mission Fleet</strong><small>{fleetActive ? 'Active · debris encounter selected' : 'Activate eight verified missions'}</small></span>{fleetActive ? <Check size={16} /> : <Plus size={16} />}</button>
        ...
        <section className="visual-workspace">
          ...
          {landing && <button className="landing-cta" onClick={activateFleet}><span className="landing-cta-kicker">EARTH OBSERVATION MISSION FLEET</span><strong>Find the debris encounter that needs attention.</strong><p>OrbitShield narrows public orbital traffic to one explainable review, then follows the protected satellite and debris to their closest approach.</p><span className="landing-cta-action">Analyse eight verified missions <ArrowRight size={17} /></span></button>}
